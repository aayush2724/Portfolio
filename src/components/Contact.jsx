...
export default function Contact() {
  ...
  const socials = [
    {
      label:'GitHub', handle:'aayush2724', href:'https://github.com/aayush2724',
      color:'#f1f0ee',
    ...
    {
      label:'LinkedIn', handle:'aayush2724', href:'https://linkedin.com/in/aayush2724',
      color:'#0a66c2',
      icon:<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>,
    },
    {
      label:'LeetCode', handle:'aayush2717', href:'https://leetcode.com/aayush2717',
      color:'#f59e0b',
      ...
      <div className="max-w-3xl mx-auto text-center">
        <div ref={ref}>
          <motion.p initial={{opacity:0}} animate={inView?{opacity:1}:{}}
            className="font-mono text-xs text-amber-500 tracking-widest uppercase mb-3">Chapter 06 — Connect</motion.p>
          ...
          <motion.p initial={{opacity:0,y:20}} animate={inView?{opacity:1,y:0}:{}} transition={{delay:.2,duration:.8}}
            ...
            Open to internships, collaborations, project ideas, or just a conversation about DSA, guitars, or building things.
          </motion.p>
        </div>

        {/* Email */}
        <motion.div initial={{opacity:0,scale:.93}} whileInView={{opacity:1,scale:1}} viewport={{once:true}} transition={{duration:.8}}
          className="mb-10">
          <motion.button onClick={copy} whileHover={{scale:1.03}} whileTap={{scale:.97}}
            ...
            <div className="relative z-10 flex items-center gap-4">
              ...
              <div className="text-left">
                <div className="font-mono text-xs text-white/25 mb-0.5">email</div>
                <div className="font-display font-semibold text-amber-400">aayush2615@gmail.com</div>
              </div>
        ...
        {/* Socials */}
        <motion.div initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:.2,duration:.8}}
          ...
          {socials.map(s => (
            <motion.a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer"
              ...
              <div className="text-left">
                ...
                <div className="text-sm font-display font-medium">{s.handle}</div>
              </div>
            </motion.a>
          ))}
        </motion.div>
        ...
        <motion.div initial={{opacity:0}} whileInView={{opacity:1}} viewport={{once:true}} transition={{delay:.4}}
          ...
          <p className="font-mono text-xs text-white/12">
            Designed & built by Aayush · {new Date().getFullYear()} · Powered by React, Tailwind & late nights ☕
          </p>
        </motion.div>
      </div>
    </section>
